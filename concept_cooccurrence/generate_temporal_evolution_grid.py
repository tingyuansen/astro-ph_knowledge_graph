#!/usr/bin/env python3
"""
Generate temporal evolution video (grid/aggregated version) using fixed-size windows.
Uses 10th percentile for color and spread-based alpha for 47×47 subcluster boxes.
...
os.makedirs('temporal_frames_grid', exist_ok=True)
...
for window_idx in tqdm(range(n_windows), desc="Generating frames"):
    ...
    # Apply percentile aggregation per subcluster box (47×47 grid)
    ...
    # Create percentile-filled matrix
    domain_matrix_median = domain_matrix.copy()
    
    for i in range(len(all_boundaries_binned) - 1):
        for j in range(len(all_boundaries_binned) - 1):
            y_start = all_boundaries_binned[i]
            y_end = all_boundaries_binned[i + 1]
            x_start = all_boundaries_binned[j]
            x_end = all_boundaries_binned[j + 1]
            
            # Extract the box region
            box_region = domain_matrix[y_start:y_end, x_start:x_end, :]
            
            if box_region.size > 0:
                # Calculate 10th percentile for color (RGB)
                p10_color = np.percentile(box_region[:, :, :3].reshape(-1, 3), 10, axis=0)
                
                # Calculate 30th percentile for comparison
                p30_color = np.percentile(box_region[:, :, :3].reshape(-1, 3), 30, axis=0)
                
                # Calculate spread as measure of consistency (average across RGB channels)
                spread = np.mean(p30_color - p10_color)
                
                # Convert spread to alpha: small spread = high alpha (solid), large spread = low alpha (transparent)
                alpha = 1.0 - np.clip(spread * 3.0, 0, 1)
                alpha = np.clip(alpha, 0.3, 1.0)
                
                # Fill the entire box with the 10th percentile color and calculated alpha
                domain_matrix_median[y_start:y_end, x_start:x_end, :3] = p10_color
                domain_matrix_median[y_start:y_end, x_start:x_end, 3] = alpha
    
    # Plot (use gaussian interpolation like visualize_cooccurrence.py)
    fig, ax = plt.subplots(figsize=(24, 20))
    ax.imshow(domain_matrix_median, aspect='auto', interpolation='gaussian', origin='upper')
    ...
    plt.tight_layout()
    plt.savefig(f'temporal_frames_grid/frame_{window_idx:04d}.png', dpi=150, bbox_inches='tight')
    plt.close()

# Create video
print("\n4. Creating video...")
subprocess.run([
    'ffmpeg', '-y', '-framerate', '2', '-i', 'temporal_frames_grid/frame_%04d.png',
    '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-crf', '23',
    'concept_cooccurrence_evolution_40k_papers_grid.mp4'
], check=True, capture_output=True)

print("="*80)
print("✅ Video generated: concept_cooccurrence_evolution_40k_papers_grid.mp4")
print("="*80)
